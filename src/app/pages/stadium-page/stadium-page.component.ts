...
export class StadiumPageComponent implements OnInit {
  ...
  openAddStadiumDialog(): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result?.name) {
        ...
        this.stadiumService
          .createStadium(stadiumData, result.image)
  ...
  openAddTribunaDialog(stadium: Stadium): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result?.nombre && result?.capacidad) {
        const newStandId = stadium.stands.length
          ? Math.max(...stadium.stands.map((s) => s.id)) + 1
          : 1; // Assign a new unique standId
        stadium.stands.push({
          id: newStandId,
          name: result.nombre,
  ...
  deleteTribuna(stadium: Stadium): void {
    stadium.stands = stadium.stands.filter(
      (stand) => stand.name !== 'Tribuna Principal',
    ); // Ensure you're filtering by stand properties
  ...
  updateStadiumImage(stadiumId: number, image: File): void {
    this.stadiumService
      .updateStadiumImage(stadiumId, image)
      .subscribe(() => this.loadStadiums());
  }
