...
export class SportMatchPageComponent {
  ...
  createPartido(partido: any) {
  // Asegúrate de que 'matchDate' no sea null antes de enviarlo
  if (!partido.matchDate) {
    partido.matchDate = new Date().toISOString(); // Asigna una fecha por defecto si es null
  }

  this.sportsMatchesService.createSportsMatch(partido).subscribe(
    (response) => {
      console.log('Partido creado:', response);
      this.loadPartidos(); // Recargar la lista de partidos
    },
    (error) => {
      console.error('Error al crear partido:', error);
      alert('Hubo un error al crear el partido. Por favor, revisa los datos.');
    }
  );
  }
